...
const produtos = {
    agua: { id: "agua", nome: "Água Mineral", retirada: 12.00, entrega: 20.00 },
    gas: { id: "gas", nome: "Gás de cozinha", retirada: 105.00, entrega: 120.00 }
};
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    document.querySelectorAll(".ADDcarrinho button").forEach((botao, index) => {
        botao.addEventListener("click", () => adicionarAoCarrinho(index === 0 ? "agua" : "gas"));
    });
    ...
    try {
        for (const id of Object.keys(produtos)) {
            ...
            if (snap.exists()) {
                ...
                atualizarProduto(id, dados, false);
            }
        }
    } catch (erro) {
        console.error("Erro ao carregar produtos do Firebase:", erro);
        console.warn("Usando os preços padrão definidos no site.");
    }
}
        ...
        <div class="tipo-pedido">
            ...
            <div class="tipo-opcoes">
                <label class="tipo-opcao">
                    ...
                    <span><strong>Retirada no local</strong><small>Preços de retirada</small></span>
                </label>
                <label class="tipo-opcao">
                    <input type="radio" name="tipo-pedido" value="entrega" ${tipoPedido === "entrega" ? "checked" : ""}>
                    <span><strong>Entrega</strong><small>Preços de entrega</small></span>
                </label>
            </div>
        </div>
...
function adicionarAoCarrinho(id) {
    ...
    else carrinho.push({ id, nome: produto.nome, quantidade: 1 });
    salvarCarrinho();
    atualizarInterface();
}
...
function removerDoCarrinho(id) {
    ...
    else carrinho = carrinho.filter(item => item.id !== id);
    salvarCarrinho();
    atualizarInterface();
}
...
function atualizarProdutosNaTela() {
    ...
    if (legendaPreco) legendaPreco.textContent = tipoPedido === "entrega"
        ? "Preços para entrega" : "Preços para retirada no local";
}
...
function atualizarCarrinho() {
    ...
    if (carrinho.length === 0) {
        lista.innerHTML = `<div class="carrinho-vazio"><span>🛒</span><p>Seu carrinho está vazio.</p><small>Adicione um produto para começar seu pedido.</small></div>`;
    } else {
        ...
        lista.querySelectorAll(".btn-remover").forEach(b => b.addEventListener("click", () => removerDoCarrinho(b.dataset.id)));
        lista.querySelectorAll(".btn-adicionar").forEach(b => b.addEventListener("click", () => adicionarAoCarrinho(b.dataset.id)));
        lista.querySelectorAll(".btn-excluir").forEach(b => b.addEventListener("click", () => removerProdutoCompletamente(b.dataset.id)));
    }

    resumo.innerHTML = `<div class="linha-total total-final"><span>Total do pedido</span><strong>${formatarMoeda(calcularTotal())}</strong></div>`;
    atualizarProdutosNaTela();
}
...
function finalizarCompra() {
    ...
    const modalidade = tipoPedido === "entrega" ? "entrega" : "retirada no local";
    let mensagem = `Olá! Gostaria de fazer um pedido para ${modalidade}:\n\n`;

    carrinho.forEach(item => {
        const produto = produtos[item.id];
        const preco = obterPrecoProduto(produto);
        mensagem += `${item.quantidade}x ${item.nome} - ${formatarMoeda(preco * item.quantidade)}\n`;
    });
...
function atualizarProduto(id, dados, atualizarCarrinho = true) {
    ...
    if (typeof dados.nome === "string" && dados.nome.trim()) produtos[id].nome = dados.nome.trim();

    // Compatibilidade com o modelo antigo.
    if (Number.isFinite(Number(dados.preco))) produtos[id].retirada = Number(dados.preco);
    if (Number.isFinite(Number(dados.retirada))) produtos[id].retirada = Number(dados.retirada);
    if (Number.isFinite(Number(dados.entrega))) produtos[id].entrega = Number(dados.entrega);
...
async function instalarAplicativo() {
    const btnInstalar = document.getElementById("btn-instalar");
    if (!installPrompt) {
        alert("Para instalar o COPAGAZ, use o menu do navegador e escolha 'Instalar aplicativo'.");
        return;
    }
    installPrompt.prompt();
    const resultado = await installPrompt.userChoice;
    if (resultado.outcome === "accepted") console.log("COPAGAZ instalado!");
    installPrompt = null;
    if (btnInstalar) btnInstalar.hidden = true;
}

window.addEventListener("appinstalled", () => console.log("COPAGAZ instalado!"));
